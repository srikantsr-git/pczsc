...
const defaultAboutUsConfig: AboutUsConfig = {
  ...
  presidentBody: `It gives me immense pleasure and pride to present the successful completion of the Pune City Zonal Sports Committee (PCZSC) Intercollegiate Sports Competitions. Every edition of these competitions reflects our collective commitment to promoting excellence in sports, encouraging healthy competition, and nurturing the all-round development of students.\n\nThis year's sporting calendar has been a remarkable journey of dedication, discipline, teamwork, and outstanding performances. Our student-athletes displayed exceptional determination, resilience, and true sportsmanship, making every event memorable. Their achievements stand as a testament to the spirit of collegiate sports and the values that sports instill in young minds.\n\nAs President of the Pune City Zonal Sports Committee for the 2026-27 academic term, I extend my heartfelt congratulations to all participating colleges, sports directors, coaches, and student-athletes. Our mission remains steadfast: to expand opportunities, elevate athletic standards, enhance infrastructure, and provide equal access for every college under Savitribai Phule Pune University. Together, we will continue to reach higher milestones of sporting excellence.`,
  presidentHighlightTitle: "Key Highlight: Introduction of Live Streaming & Digital Infrastructure",
  presidentHighlightBody: "One of the significant highlights this year was the introduction of Live Streaming of selected sporting events along with digital draws and automated result publishing. This initiative enabled students, faculty members, parents, alumni, and sports enthusiasts to witness the competitions live from anywhere, ensuring complete transparency and accessibility.",
  presidentName: "Dr. Sachin Sakhre",
  presidentPhoto: defaultBlankAvatar,
  ...
  useEffect(() => {
    async function hydrateStores() {
      try {
        ...
        let targetAbout = dbAbout || lsAbout || getUserStateFallback('pczsc_about_cfg', defaultAboutUsConfig);
        if (targetAbout.presidentName?.includes('Iqbal') || (targetAbout.presidentBody && targetAbout.presidentBody.split('\n\n').length < 2)) {
          targetAbout = defaultAboutUsConfig;
        }
        ...
        const isStaleCommittee = mergedCom.some(m => m.name.includes('Iqbal') || (m.id === 'cm-1' && !m.name.includes('Sakhre')));
        const targetCom = (!mergedCom.length || isStaleCommittee) ? initialCommitteeMembers : mergedCom;
        const hydratedCom = await hydrateImagesFromIDB(targetCom);
